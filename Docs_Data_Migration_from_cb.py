...
def get_audio_objects(mapped_obj, cb_obj, cdn):
    ...
    if 'transcriptPath' in media_obj and media_obj['transcriptPath'] is not None:
        mobj = copy.deepcopy(mapped_obj)
        mapped_list.append(
            get_transcription_media(mobj, media_obj, region_bucket[cdn], media_obj.get('vttSubtitlePath')))
    ...
    if 'original_path' in media_obj and media_obj['original_path'] is not None:
        ...
        if media_obj.get('uuid') or media_obj.get('docProcessor') == 'BOX' or media_obj.get('docProcessor') == 'HYBRID':
            if media_obj.get('docProcessor') == 'HTML_PDF_LAMBDA':
                pdf = get_mapped_document_obj(copy.deepcopy(mapped_obj), media_obj, region_bucket[cdn],
                                              original_path + "/doc.pdf")
                thumb = get_mapped_image_object(copy.deepcopy(mapped_obj), media_obj, croco_bucket[cdn],
                                                original_path + "/imagified/out_1.png")
            ...
            if media_obj.get('docProcessor') == 'CROCODOC' or media_obj.get('docProcessor') is None:
                ...
                pdf = get_mapped_document_obj(copy.deepcopy(mapped_obj), media_obj, croco_bucket[cdn],
                                              uuid + "/doc.pdf")
                thumb = get_mapped_image_object(copy.deepcopy(mapped_obj), media_obj, croco_bucket[cdn],
                                                uuid + "/images/thumbnail-master-0.png")
                mapped_list.append(pdf)
                mapped_list.append(thumb)

            else:
                pdf = get_mapped_document_obj(copy.deepcopy(mapped_obj), media_obj, box_bucket[cdn],
                                              obj_id + "/doc.pdf")
                thumb = get_mapped_image_object(copy.deepcopy(mapped_obj), media_obj, box_bucket[cdn],
                                                obj_id + "/imagified/out_1.png")
                mapped_list.append(pdf)
                mapped_list.append(thumb)
...
def get_companies_by_type(comp_type='ALL'):
    ...
    companies = [comp.strip('.settings') for comp in companySettings if
                 companySettings[comp]['companyType'] == comp_type]
    return companies
